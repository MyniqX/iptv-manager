...
export default function mkvExtract(file: File, callback: Callback): void {
...
function handleStream(stream: ReadableStream, callback: Callback): void {
        ...
        switch (chunk[0]) {
            ...
            case "tag":
                if (chunk[1].name === "FileName") {
                    if (!files[currentFile]) files[currentFile] = new File();
                ...
                if (chunk[1].name === "BlockDuration" && trackIndex !== -1) {
                    ...
                    trackData[trackIndex].push(duration.toString());
                }
                break;
            default:
            // do nothing
...
function padZeroes(arr: Uint8Array): ArrayBuffer {
    ...
    const output = new Uint8Array(len % 2 === 0 ? len : len + 1);
    output.set(arr, len % 2 === 0 ? 0 : 1);
    return output.buffer;
}

function readUnsignedInteger(data: ArrayBuffer): number {
    const view = new DataView(data);
    return view.getUint8(0) + (data.byteLength === 2 ? view.getUint8(1) * 256 : 0);
}
