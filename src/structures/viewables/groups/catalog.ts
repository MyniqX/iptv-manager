...
import { M3UObject } from "@/utils/m3uobject";
import { WatchableObject } from "../watchables/watchobject";
import { EList } from "@/utils/elist";
import { M3UFileParser } from "@/utils/m3ufileparser";
...
export class Catalog extends GroupObject {
    ...
    get GroupCount(): number {
        ...
        this.movieList.Groups.map(m => gset.add(m.Name));
        this.tvShowList.Groups.map(m => gset.add(m.Name));
        this.liveStreamList.Groups.map(m => gset.add(m.Name));
        return gset.size;
    }
